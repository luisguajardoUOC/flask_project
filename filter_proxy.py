...
class ProxyFilter:
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        client_id1= id(client_ip)
        client_id = id(flow.client_conn) 
        if client_id not in self.connections:
            # Registra una nueva conexión si ha sido eliminada previamente
            logging.info(f"Creando nueva conexión para el cliente {client_ip} (ID: {client_id})")
            self.create_connection(client_id, client_ip)
    ...
    def create_connection(self, client_id, client_ip):
        ...
        logging.info(f"Nueva conexión creada para IP {client_ip} con ID {client_id}")
